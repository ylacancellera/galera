...
namespace gcache
{
    ...
    private:
        ...
        int master_key_id_;
        gu::UUID const_mk_id_;  // Part of MK id. This is to easier
                              // identify keys generated by the same GCache in keyring vault
        gu::UUID master_key_uuid_;
        std::string file_key_;
        gu::MasterKeyProvider *master_key_provider_;
        gu::RecursiveMutex mk_rotation_mutex_;
        bool encrypt_;
        ...
        static std::string const PR_KEY_MK_CONST_ID;
        ...
        std::string   generate_new_master_key(const std::string& key_name);
